...
const HelpSection: React.FC<HelpSectionProps> = ({
  ...
  return (
    <div className="portal-panel rounded-[2rem] p-8 border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-all h-full flex flex-col">
      <div
        className={`size-14 rounded-2xl ${iconColor} flex items-center justify-center mb-6`}
      >
        <span className="material-symbols-outlined text-3xl">{icon}</span>
      </div>
      <h3 className="text-2xl font-extrabold text-[#120e1b] dark:text-white mb-3 tracking-tight">
        {title}
      </h3>
      ...
      <div className="space-y-3 mt-auto">
        {questions.map((faq, idx) => (
          ...
          >
            ...
            >
              <span
                className={`text-sm font-bold transition-colors ${expandedIdx === idx ? "text-primary" : "text-[#120e1b] dark:text-white group-hover:text-primary"}`}
              >
                {faq.q}
              </span>
              <span
                className={`material-symbols-outlined transition-transform duration-300 ${expandedIdx === idx ? "rotate-180 text-primary" : "text-gray-400"}`}
              >
                expand_more
              </span>
            </button>
            <div
              className={`transition-all duration-300 ease-in-out ${
                expandedIdx === idx
                  ? "max-h-60 opacity-100 pb-5 px-5"
    ...
    <div className="min-h-full bg-[#f8faff] dark:bg-background-dark font-sans flex flex-col">
      ...
      <main className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-10 sm:py-16 space-y-10 sm:space-y-16">
        ...
        <div className="bg-[#120e1b] dark:bg-primary/10 rounded-[2rem] sm:rounded-[3rem] p-8 sm:p-16 text-center relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-400 font-medium mb-10 max-w-xl mx-auto text-lg">
              Our Hawassa-based support team is ready to assist you. We usually
        ...
        <div className="space-y-10">
          <h2 className="text-3xl font-extrabold text-[#120e1b] dark:text-white text-center tracking-tight">
            Community Standards
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div
                key={i}
                className="portal-panel rounded-3xl p-8 border border-gray-100 dark:border-gray-800 hover:border-primary/30 transition-colors"
              >
                <h4 className="text-sm font-bold text-primary mb-3 uppercase tracking-widest">
                  {faq.q}
                </h4>
