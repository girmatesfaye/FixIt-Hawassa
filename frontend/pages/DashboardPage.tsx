            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div className="flex items-center gap-5">
                ...
                >
                  <img
                    src={
                      getUploadedImageUrl(currentUserAvatar) ||
                      `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(currentUserName || "User")}`
                    }
                    alt="Profile"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3 sm:gap-5">
            {(() => {
              ...
              categories.forEach((dynamic) => {
                if (
                  !merged.find(
                    (m) => m.name.toLowerCase() === dynamic.name.toLowerCase(),
                  )
                ) {
                  merged.push(dynamic);
                }
