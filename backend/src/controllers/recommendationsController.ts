...
export const rankRecommendations = async (req: Request, res: Response) => {
...
export const getRecommendationsForRequest = async (
  req: Request,
  res: Response,
) => {
  const parsedQuery = querySchema.safeParse(req.query);
  ...
  let snapshotDoc = await RecommendationSnapshot.findOne(snapshotQuery).lean();
  ...
  if (!normalizedSnapshotRecommendations) {
    ...
    const changed = normalizedSnapshotRecommendations.some(
      (recommendation, index) =>
        ...
        String(snapshotRecommendations[index]?.id),
    );
